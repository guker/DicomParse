...
static __inline  unsigned char  readByte(unsigned char *lRawRA, long *lRawPos, long lRawSz) {
...
//with Clang/LLVM there is no benefit for "static __inline" for any of the following functions
...
//int bitMask(int bits) {    return ( (2 << (bits - 1)) -1); }
static const int bitMask[16] = {0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767 };
...
unsigned char *  decode_JPEG_SOF_0XC3 (char *data, int BytesCount, bool verbose, int *dimX, int *dimY, int *bits, int *frames) {
    #define abortGoto() free(lRawRA); return NULL;
    unsigned char *lImgRA8 = NULL;
   
    long lRawSz = BytesCount;
    unsigned char* lRawRA = (unsigned char*)data;
